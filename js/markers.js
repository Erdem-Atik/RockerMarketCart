  ...
  pdata.items.forEach((el, index) => {
    const mark = `
      ...
      <div class="product" id="${el.sys.id}" >      
        <img class="product-image" src="./images/product-${
          index + 1
        }.JPG" alt="product ${index + 1}" />
...
const displayCart = function (cartData) {
  cartContainer.innerHTML = "";
  const uniqCartData = cartData.filter(
    (
      el,
      index,
      self //select only uniq elements in the array(cart-JSON data) in order to mark
    ) => index === self.findIndex((it) => it.sys.id === el.sys.id)
  );
  if (uniqCartData) {
    uniqCartData.forEach((el) => {
      const markup = `<li class="cartItems">
              <div class="item">
                  <div class="cart">
                   ...
                   <form>
                    <div class="decrease value-button" id="${
                      el.sys.id
                        ...
                        <input type="number" id="number" value="${quantityOfEachProduct(
                          +el.sys.id,
                          cart
                        )}"/>
                    <div class="increase value-button" id="${
                      el.sys.id
                  ...
                  <div class="price">
                   <h5 class="productPrice">$${
                     quantityOfEachProduct(+el.sys.id, cart) * +el.fields.price
                   }</h5>
                   </div>
              </div>
             </li>
